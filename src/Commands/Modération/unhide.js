...
exports.run = async (client, message, args) => {
    ...
    try {
        if ([ChannelType.GuildText, ChannelType.GuildVoice, ChannelType.GuildCategory, ChannelType.GuildAnnouncement, ChannelType.GuildForum].includes(targetChannel.type)) {
            ...
            return message.channel.send(`Le salon ${targetChannel} a été affiché.`);
        } else {
            return message.channel.send("Le type de salon spécifié ne peut pas être affiché avec cette commande.");
        }
    } catch (error) {
        console.error("Erreur lors de la modification des permissions:", error);
        return message.channel.send("Une erreur s'est produite lors de la tentative d'afficher le salon.");
    }
};
